...
* A new option has been added in the Error management properties of workflow's activities: The Abort on error option will stop automatically the workflow. You will not be able to restart it afterwards. (NEO-29661)

**Patches**

* Fixed an issue which lead to a bad request error when using xml-data field with the Microsoft Dynamics CRM account if the referenced xml contained double quotes.
...
* Fixed an issue which prevented Campaign templates to be deleted.
...
* Fixed an issue which prevented the Hot Clicks report to load for deliveries since build 9182. (NEO-28900)
...
* Fixed an error that prevented delivery targeted to Seed addresses to be sent when using Seed member properties in Personalization fields.
...
* Fixed an issue that caused network timeout issues to be incorrectly logged as script interruption issues instead of network errors. This issue occurred in the case of HTTP requests that are included in JavaScript activities. (NEO-38079)
* Fixed an issue that could prevent the preview of email deliveries that are attached to a workflow. (NEO-37840)
* Fixed an issue that could cause valid tables that contain list values to be deleted by the database cleanup workflow. (NEO-34911)
...
* Fixed an error that prevented the delivery logs to be updated correctly when sending with the Campaign MTA: email messages were still showing as failed even though they were properly delivered. (NEO-33449)
...
* Fixed an error that prevented personalized tracking URLs to be redirected when the URL signature mechanism for tracking links was enabled.
