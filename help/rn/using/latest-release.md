...
* Fixed an issue which displayed an error message when uploading a CSV file when using Campaign Classic 9080 build connected to a server using an older build.
* Fixed an issue that could display an error message when configuring Microsoft Dynamics CRM wizard for an external account. This was due to a compatibility issue with the latest MS Dynamics CRM API version.
* Fixed an issue that prevented you from exporting look up-type records (meaning data made up of foreign key records connected with other tables) from Campaign Classic to Microsoft Dynamics using the CRM connector.
* Fixed an issue that could prevent Microsoft Dynamics data from being fetched if the **Automatic index** option was enabled in the CRM connector.
* Fixed an issue with IMS authentication that could leave connections opened even if they were ended. Terminated connections will now be automatically closed as soon as they are ended in order to avoid accumulating connections and consuming system resources.
...
Implements Customer request NEO-26856
* Fixed an issue when using the **File Transfer** activity linked to an Azure external account encrypted in SSL where the connection was made with HTTP instead of HTTPS.
...
* Fixed a crash which occurred during the web process shutdown if Interaction requests were still being processed.
...
* Fixed an issue where the **NoNull** function in Oracle DB was not working anymore after upgrade 9032.
