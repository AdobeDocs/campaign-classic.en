...
uuid: ddd279fb-629f-4b55-b8f6-f49da4fdcab5
contentOwner: sauviat
products: SG_CAMPAIGN/CLASSIC
content-type: reference
audience: reporting
discoiquuid: 814d7af6-873f-4ecd-bf38-c627016fa4c4
index: y
internal: n
snippet: y
---
...
In the first step of the assistant, you can select the descriptive analysis template to use. By default, Adobe Campaign offers two templates: **Qualitative distribution** and **Quantitative distribution**. For more on this refer to the [Configuring the qualitative distribution template](../../reporting/using/use-cases.md#configuring-the-qualitative-distribution-template) section. The various renderings are presented in the [About descriptive analysis](../../reporting/using/about-descriptive-analysis.md) section.
...
Possible configurations of the different graphics are detailed in the [Analysis report chart options](../../reporting/using/use-cases.md#analysis-report-chart-options) section.
...
To create a new statistic, click the **Add** button. For more on this, refer to [Statistics calculation](../../reporting/using/use-cases.md#statistics-calculation).
...
1. Then select the **2 variables** option and specify that the **First variable (abscissa)** will refer to recipient status (prospects/customers) and the second variable will refer to the country.
1. Select **Cylinders** as a type.

   ![](assets/s_ncs_user_report_wizard_05.png)

1. Click **Next** and leave the default **Simple count** statistic.
1. Click **Next** to display the report.
...
1. Select the **Company** variable as a column and the **Email domain** variable as a row.
1. Keep the **By rows** option for statistics orientation: the statistic calculation will be displayed to the right of the **Email domain** variable.

   ![](assets/s_ncs_user_report_wizard_03b.png)

   Click **Next** to approve this step.

1. Then enter the statistics to be calculated: keep the default count and create a new statistic. To do this, click **Add** and select **Total percentage distribution** as the operator. 
...
1. Once the analysis report has been generated, you can adapt the display to suit your needs without changing the configuration. For instance, you can switch the axes: right-click the domain names and select **Turn** on the shortcut menu.
...
1. Select the **Age** variable and enter its label. Specify whether or not it's an integer, then click **Next**.

   ![](assets/s_ncs_user_report_wizard_011.png)

1. Delete the **Deciles**, **Distribution** and **Sum** statistics: they're not needed here.
...
1. Select **Analyze target** in the drop-down menu to open the descriptive analysis window.

   ![](assets/s_ncs_user_report_wizard_from_transision.png)

1. At this point you can either select the **Existing analyses and reports** option and use reports created previously (refer to [Re-using existing reports and analyses](../../reporting/using/use-cases.md#re-using-existing-reports-and-analyses)), or create a new descriptive analysis. To do this, leave the **New descriptive analysis from a template** option selected by default.
...
1. Add a statistic for counting the number of clicks: define the relevant filter in the **Filter** tab.

   ![](assets/reporting_descriptive_sample_tracking_3.png)

1. Then click the **General** tab to rename the statistics label and alias:
...
1. Then click the **General** tab to rename the statistics label and its alias:

   ![](assets/reporting_descriptive_sample_tracking_6.png)

1. Add the third statistic and select the **Calculated field** operator to measure the reactivity rate.
