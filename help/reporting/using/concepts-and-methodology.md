...
uuid: 96fb7c2e-a1a2-47e9-a280-b926a00d6ba7
contentOwner: sauviat
products: SG_CAMPAIGN/CLASSIC
content-type: reference
audience: reporting
discoiquuid: 8f017c7c-ba6b-4574-a61c-74dd9b7426d1
index: y
internal: n
snippet: y
---
...
To create each bin individually, select the **Define each bin** option and use the table to create the various bins.

![](assets/s_advuser_cube_class_01.png)

Click the **Add** button to create a new bin and list the values which will be grouped into the bin.
...
You can use an SQL mask to combine several values into a filter. To do this, check **Yes** in the **Use an SQL mask** column and enter the SQL filter to be applied in the **Value or expression** column.
...
1. Select the **Dynamically link the values to an enumeration** option and select the matching enumeration.
...
To define ranges manually, click the **Add** button and select **Define a range**:

![](assets/s_advuser_cube_class_05.png)

Then specify the lower and upper limits and click **Ok** to confirm.

### Generating bins automatically {#generating-bins-automatically}

It's also possible to generate bins automatically. To do this, click the **Generate bins...** link.
...
To create a report using this enumeration, create a Cube using the **Email domain** dimension. Choose the **Enable binning** option then **Dynamically link the values to an enumeration**. Then select the **Domains** enumeration as shown above. All values that have no specified alias will be regrouped under the **Others** label.
...
The **Domains** enumeration is used to generate built-in reports that display the list of domains. To adapt the content of these reports, you can edit this list.
...
1. Click the **Aggregates** tab of the cube, then click the **Add** button.
...
1. Click the **Workflow** tab to create the aggregation workflow.

   ![](assets/s_advuser_cube_agregate_04.png)

    * The **Scheduler** activity lets you define the frequency of calculation updates. The scheduler is detailed in [this section](../../workflow/using/scheduler.md).
    * The **Aggregate update** activity lets you select the update mode which you want to apply: full or partial.
...
The types of measures are defined in the **Measures** tab of the cube. You can calculate sums, averages, deviations, etc.
...
1. Click the **Add** button above the list of measures and select the type of measure and the formula to be calculated.
...
1. If necessary, and depending on the operator, choose the expression which the operation concerns.
   ...
   The **Advanced selection** button lets you create complex calculation formulas. For more on this, refer to [this section](../../platform/using/about-queries-in-campaign.md).

   ![](assets/s_advuser_cube_create_a_measure_01.png)

1. The **Filter the measure data...** link lets you restrict the calculation field and only apply it to specific data in the database.

   ![](assets/s_advuser_cube_create_a_measure_02.png)

1. Enter the label of the measure and add a description, then click **Finish** to create it.
...
The measures calculated in the cube are configured via the **Measures** button.
...
* Click the **Edit the configuration of the pivot table** icon to access the configuration window.
...
To do this, click the **Show the report properties** icon and enable the **Share this report** option.
...
Filters can also be created based on a selection. To do this, select your source cells, lines and columns, then click the **Add a filter** icon.
