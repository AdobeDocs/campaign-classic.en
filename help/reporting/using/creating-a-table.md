...
uuid: eb3d8c4f-82b7-4439-87d5-e9117e7e3eac
contentOwner: sauviat
products: SG_CAMPAIGN/CLASSIC
content-type: reference
audience: reporting
discoiquuid: b0e232c8-f926-44d1-8c9e-302d51774612
index: y
internal: n
snippet: y
---
...
A **List with group** type table lets you group data in the table and produce statistics on it. For instance, you can create totals and sub-totals for the data. Each group has its own header, detail and footer line.

>[!CAUTION]
>
>The **Page** activity containing the table must be preceded by a **Query** or **Script** activity to collect the data to be analyzed in the report. For more on these activities, refer to [Collecting data to analyze](../../reporting/using/collecting-data-to-analyze.md) and [Script activity](../../reporting/using/creating-a-table.md#script-activity).
...
It lets you list the campaigns (**Label (Campaign)**, the list of deliveries (**Label**) linked to the campaign, and lets you count the number of messages sent per delivery (**Processed)**, before adding them up for each campaign (**Sum(@processed)**). 
...
1. Go to the report chart and place a **Query** activity. Refer to [Collecting data to analyze](../../reporting/using/collecting-data-to-analyze.md). 
1. Fill in the source table and select the fields of the table which the statistics will concern. 
1. Place a **Page** activity in the chart. For more on this, refer to [Static elements](../../reporting/using/creating-a-table.md#static-elements). 
1. Insert a **List with group** type table into the page. 
...
1. Display the finalized report in the **Preview** tab. You can then publish the report and export it into a different format if necessary. For more on this, refer to [Exporting a report](../../reporting/using/creating-a-table.md#exporting-a-report).
...
Use the **Expression** menu entry to select the values to display.
...
* Enter a label for a cell, the header one for example.
  ...
  To do this, use the same process as for inserting a field into the database, but don't select an expression. Enter the label in the **Label** field. It will be displayed as is.

* Calculating an aggregate (an average, a sum, etc.) and displaying it in the cell.

  To do this, use the **Aggregates** menu entry and select the desired campaign.
...
To define the cell format, the **Cell format...** menu lets you access all formatting options available for the selected cell.

These options enable you to personalize the final rendering of the report and make it easier to read information.

Use the **Carriage return** field when exporting data to Excel: select the **Yes** value to force the carriage return. This value will be kept when exporting. For more on this, refer to [Exporting a report](../../reporting/using/creating-a-table.md#exporting-a-report).

The **Cell format** window lets you access the following tab:
...
The **Value** tab lets you change the font and the various value attributes or to define a format based on their nature. 

![](assets/s_advuser_ergo_listgroup_009.png)

The format changes data display: for example, the **Number**, **Monetary** and **Percentage** formats allow you to align the figures on the right and display decimal points.
...
If necessary, you can define borders in the table template (**Administration > Configuration > Form rendering**).
...
The **Click** tab lets you define an action when the user clicks the content of a cell or of the table.
...
Create a new report that concerns the campaign schema, **Campaigns (nms)**.
...
1. Start by changing the schema on which the query's source is applied: select the **Deliveries (nms)** schema.
1. Click the **Edit query** link and display the advanced fields.
...
1. Click the **Next** button twice to get to the **Data filtering** step.
...
1. Click **Finish** to save these conditions, then click **Ok** to close the query editor.
...
1. Open the **Page ** activity and enter its title, for instance **Deliveries** in this case.
...
1. Click the **Table data XPath...** link and select the delivery link, i.e. [query/delivery]

   ![](assets/s_advuser_report_listgroup_005.png)

1. Click the **Data** tab and change layout of the table: add three columns on the right.
...
1. Edit the first cell of the group header and insert the **Label** field of the campaigns as an expression.

   ![](assets/s_advuser_report_listgroup_009.png)

1. Edit the second cell of the details line and select the deliveries **Label**.

   ![](assets/s_advuser_report_listgroup_011.png)

1. Edit the format of this cell and open the **Click** tab. Configure the adequate options so that when the users clicks the name of a delivery, it opens in the same window. 

   ![](assets/s_advuser_report_listgroup_0111.png)

   To do this, select a **Next page** type action and select **In the same window** as an open option.

   ![](assets/s_advuser_report_listgroup_0112.png)

1. In the lower section of the window, click **Add** and specify the

   ```
   /vars/selectedDelivery
   ```

   path and the **@deliveryId** expression that matches the alias of the primary key of the delivery, as defined in the query created previously. This formula lets you access the selected delivery.

   ![](assets/s_advuser_report_listgroup_010.png)

1. Edit the second cell of the footer line of the group and enter **Total per campaign** as a label.

   ![](assets/s_advuser_report_listgroup_012.png)

1. Edit the third cell of the header line of the group and enter **Number of messages sent** as a label.
...
1. Edit the third cell of the footer line of the group, select the processed delivery indicator and apply the **Sum** aggregate to it.
...
1. Select this cell to display a value bar representing the delivery error rate.
   ...
   To do this, access the cell format, then go to the **More** tab. Select the **Value bar** entry in the drop-down list and select the **Hide the cell value** option.

   ![](assets/s_advuser_report_listgroup_023.png)

   You may now view a rendering of the report. Click the **Preview** tab and select the **Global** option: this shows the list of all deliveries in the Adobe Campaign database that are linked to a campaign.

   ![](assets/s_advuser_report_listgroup_025.png)

   We recommend using the **Preview** tab to make sure the data in your table is properly selected and configured. Once this is done, you can go on to formatting your table.

1. Apply the **Bold** style to the cells that show the total per campaign and the total number of messages processed.

   ![](assets/s_advuser_report_listgroup_024.png)

1. Click the 1st cell of the group header line, the one that displays the campaign name, and select **Edit > Merge to right**.
...
1. Add a new query after the **Page** activity and edit its schema: select the **Recipient delivery logs** schema.
...
1. Edit the query and define the output columns. To display the number of deliveries per email domain, you need to:
    ...
    * collect recipient email domains and group information on this field: to do this, select the **Group** option in the domain name column.
...
1. Click the **Next** button twice: this takes you to the **Data filtering** step.
...
1. Uncheck the **Enable output transitions** option: this is the last page of the report and will not be followed by another activity.
...
1. Click the **Table data XPath...** and select the **Recipient delivery logs** link.

   ![](assets/s_advuser_report_listgroup_029.png)

1. In the **Data** tab, adapt the table as follows:

    * Add two columns on the right hand side.
    * In the first cell of the detail line, add the **rowNum()-1** expression to count the number of lines. Then alter the format of the cell: in the **Extra** tab, select **Color tab** and click **Ok**.
    
      ![](assets/s_advuser_report_listgroup_018.png)

      This configuration will enable you to use the table as a caption for the chart.
    
    * In the second cell of the detail line, add the **Email domain(Recipient)** expression.
    * In the third cell of the detail line, add the **count(primary key) **expression.

   ![](assets/s_advuser_report_listgroup_019.png)

1. Add a pie chart to the page using the right-click menu and assign the **Email domains** label to it. For more information, refer to [Chart types and variants](../../reporting/using/creating-a-table.md#chart-types-and-variants).
1. Click the **Variants** link and deselect the **Display label** and the **Display caption** options. 
...
1. In the **Data** tab, change the data source: select **Context data** from the drop-down list.

   ![](assets/s_advuser_report_listgroup_020.png)

1. Then click **Advanced settings** and select the link to the recipient delivery logs.

   ![](assets/s_advuser_report_listgroup_0201.png)

1. In the **Chart type** section, select the **Email domain** variable.
1. Then add the calculation to be carried out: select the sum as an operator.

   ![](assets/s_advuser_report_listgroup_0202.png)

1. Click the **Detail** button to select the field which the count will concern, then close the configuration window.
...
To view the result of this configuration, click the **Preview** tab and select the **Global** option.
