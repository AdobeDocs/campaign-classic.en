...
### Use case: Importing data encrypted using a key generated by Control Panel
...
1. Open the **[!UICONTROL File transfer]** activity then specify the external account form which you want to import the encrypted .gpg file.
...
1. Open the **[!UICONTROL Data loading (file)]** activity, then configure it according to your needs. Global concepts on how to configure the activity are available in [this section](../../workflow/using/data-loading--file-.md).
    ...
    >[!NOTE]
    >
    >In this example, we are using the passphrase used by default by Control Panel ("passphrase"). You can change this parameter's value according to your needs:
    >
    >`gpg --batch --passphrase my_passphrase -- decrypt <%=vars.filename%>`
