...
uuid: b5303347-69f3-4b55-a0dc-93c018321ed5
contentOwner: sauviat
products: SG_CAMPAIGN/CLASSIC
content-type: reference
discoiquuid: 1ca47e2e-669a-4b4a-a0dc-2602de819c41
index: y
internal: n
snippet: y
---
...
A **Query** activity lets you select basic data to build the target population. For more on this, refer to [Creating a query](../../workflow/using/targeting-data.md#creating-a-query).
...
It is possible to collect additional data to be forwarded and processed throughout the workflow's life cycle. For more on this, refer to [Adding data](../../workflow/using/targeting-data.md#adding-data) and [Editing additional data](../../workflow/using/targeting-data.md#editing-additional-data).
...
Data reconciliation is carried out either via an identifier, changing axis, or a join. An example is available in [Using data from a list: Read list](../../workflow/using/targeting-data.md#using-data-from-a-list--read-list).
...
1. In your workflow, drag and drop a **Split** activity.
1. In the **General** tab, select the desired option: **Use data from the target and additional data**, **Use the additional data only** or **Use external data**.
1. If the **Use data from the target and additional data** option is selected, the targeting dimension lets you use all the data conveyed by the inbound transition.
   ...
   An example showing how to use filtering conditions in the **Split** activity to segment the target into different populations is described in [this section](../../workflow/using/cross-channel-delivery-workflow.md).
...
1. If the **Use the additional data only** option is selected, only additional data is offered for subset filtering.

   ![](assets/split-subset-config-additional-data-only.png)

1. If the **Federated Data Access** option is enabled, the **Use external data** lets you process data in an external database which is already configured, or create a new connection to a database.
...
1. Click the **Add** button and define the filtering conditions. 

   ![](assets/wf_split_add_a_tab.png)

1. Define the filtering dimension in the **General** tab of the activity (see above).It applies to all subsets by default. 
...
1. If necessary, you can change the filtering dimension for each subset individually. This lets you build a set for all Gold card holders, one for all recipients who clicked in the latest newsletter and a third for people aged 18 to 25 who made an in-store purchase within the last 30 days, all using the same split activity. To do this, select the **Use a specific filtering dimension** option and select the data filtering context.
   ...
   >[!NOTE]
   >
   >If you have acquired the **Federated Data Access** option, you can create subsets based on the information in an external base. To do this, select the schema of the external table in the **Targeting dimension** field. For more on this, refer to [Accessing an external database (FDA)](../../workflow/using/accessing-an-external-database--fda-.md).
...
Subsets can also be created using the **Cells** activity. For more on this, refer to the [Cells](../../workflow/using/cells.md) section.
...
In addition to the targeting dimension, the filtering dimension lets you specify the nature of the collected data. Refer to [Targeting and filtering dimensions](../../workflow/using/targeting-data.md#targeting-and-filtering-dimensions).
...
* The **Enrichment** activity lets you momentarily add columns to a schema, as well as add information to certain elements. It is detailed in the [Enrichment](../../workflow/using/enrichment.md) section of the repository of activities.
* The **Edit schema** activity lets you modify the structure of a schema. It is detailed in the [Edit schema](../../workflow/using/edit-schema.md) section of the repository of activities.
* The **Change dimension** activity lets you change the targeting dimension during the target construction cycle. It is detailed in the [Change dimension](../../workflow/using/change-dimension.md) section.
