...
|**Calculation of Twitter statistics** (statsTwitter)|Social networks (Social Marketing) - Campaign v7 only|This workflow calculates statistics linked to retweets and visits on X (formerly known as Twitter).|
...
|**Database cleanup** (cleanup)|Delivery|This workflow is the database maintenance workflow: it makes different calculations from the statistics and processes, and deletes obsolete data from the database according to the defined configuration in the deployment wizard. It is triggered every day at 4am by default. For more information, refer to [this page](../../production/using/database-cleanup-workflow.md#monitoring-campaign-classic).|
...
|**Synchronizing Twitter pages** (syncTwitter)|Social networks (Social Marketing) - Campaign v7 only|This workflow imports X followers into Adobe Campaign every day at 7am.|
...
|**Refresh for deliverability** (deliverabilityUpdate)|Delivery|This workflow runs nightly and manages the bounce emails qualification rules, as well as the list of domains and MXs. This requires the HTTPS port to be open on the platform.|