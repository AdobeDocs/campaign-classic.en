...
uuid: b156875f-bfac-4a51-a29c-726ed043d2b9
contentOwner: sauviat
products: SG_CAMPAIGN/CLASSIC
content-type: reference
discoiquuid: d93ece9d-b0dc-48d6-a281-a905bb589103
index: y
internal: n
snippet: y
---
...
>* This maintenance operation can be implemented by an Adobe Campaign workflow, using an **SQL** activity (for more on this, refer to [this section](../../workflow/using/executing-a-workflow.md#architecture)). Make sure you schedule maintenance for a low activity time which does not collide with your backup window.
>
...
1. Go to the **Management > Maintenance Plans** folder, right-click on it and choose **Maintenance Plan Wizard**
1. Click **Next** when the first page comes up.
1. Select the type of maintenance plan you want to create (separate schedules for each task or single schedule for the whole plan), then click the **Change...** button.
1. In the **Job schedule properties** window, select the desired execution settings and click **OK**, then click **Next**.
1. Select the maintenance tasks you want to perform, then click **Next**.
...
1. In the drop-down list, select the database which you want to run the **Database Check Integrity** task on.
1. Select the database and click **OK**, then click **Next**.
1. Configure the maximum size allocated to your database, then click **Next**.
...
1. Reorganize or rebuild the index:
    ...
    * If the index fragmentation rate is higher than 40%, a rebuild is recommended.
      ...
      >[!NOTE]
      >
      >The rebuild index process is more constricting in terms of processor use and it locks the database resources. Tick the **Keep index online while reindexing** option if you want the index to be available during the rebuild.

1. Select the options you want to display in the activity report, then click **Next**.
1. Check the list of tasks configured for the maintenance plan, then click **Finish**.

   A summary of the maintenance plan and the statuses of its various steps is displayed.

1. Once the maintenance plan is complete, click **Close**.
1. In the Microsoft SQL Server explorer, double-click the **Management > Maintenance Plans** folder.
