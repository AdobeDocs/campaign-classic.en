...
uuid: 545e714e-5b15-451b-b3ce-de68a8319bef
contentOwner: sauviat
products: SG_CAMPAIGN/CLASSIC
audience: interaction
content-type: reference
discoiquuid: ce8a4758-3cb4-4e36-8495-ce38f8697b68
index: y
internal: n
snippet: y
---
...
The content of the catalog offers is configured in the offer spaces. By default, the content can include the following fields: **Title**, **Destination URL**, **Image URL**, **HTML content** and **Text content**. The field sequence is configured in the offer space.

Advanced parameters let you specify a contact identification key (which can be made of up of various elements, the name and email field at the same time for example). For more on this, refer to the [Presenting an identified offer](../../interaction/using/creating-offer-spaces.md#presenting-an-identified-offer) section.
...
1. Go to the list of offer spaces and click **New**.
...
1. Check the **Enable unitary mode** box if one of the following cases applies to you:

    * You are using Interaction with Message Center
    * You are using Interaction's unitary mode (inbound interactions)

1. Go to the **Content field** window and click **Add**.

   ![](assets/offer_space_create_003.png)

1. Go to the **Content** node and select the fields in the following order: **Title**, then **Image URL**, then **HTML content**, then **Destination URL**.

   ![](assets/offer_space_create_004.png)

1. Check the **Required** box to make each field mandatory.
...
1. Click **Edit functions** to create a rendering function.
...
1. Go to the **HTML rendering** tab and select **Overload the HTML rendering function**.
...
When an offer proposition is created by the interaction engine its status is changed, whether it is an inbound or an outbound interaction. The choice between these two values depends on the way the offer spaces were configured in the **Design** environment
...
1. Go to the **Storage** tab of the desired space.
...
As soon as a visitor clicks the offer, and therefore the URL, the **Accepted** status (value **3**) is applied to the proposition and the visitor is redirected to a new page of the **Neobank** site to take out the insurance contract.

>[!NOTE]
>
>If you want to specify another status in the url (for example if an offer proposition is rejected), use the value corresponding to the desired status. Example: **Rejected** = "5", **Presented** = "1" and so on.  
>Statuses and their values can be retrieved in the **Offer propositions (nms)** data schema. For more on this, refer to [this page](../../configuration/using/data-schemas.md).

**Outbound interaction**

In case of an outbound interaction, you can automatically apply the **Interested** status to an offer proposition when the delivery contains a link. Simply add the **_urlType="11"** value to the link:
