...
uuid: caab0eff-a028-4d1f-a67f-11a85c0304a5
contentOwner: sauviat
products: SG_CAMPAIGN/CLASSIC
content-type: reference
discoiquuid: dcac67aa-59c1-4ce7-b00b-e1c9c25d5577
index: y
internal: n
snippet: y
---
...
1. In the **Connector** tab, fill in the **Access URL of the connector** field:
...
1. Go to **Administration > Platform > External accounts**.
1. Select the **Android routing** external account.
1. In the **Connector** tab, fill in the **JavaScript used in the connector** field:
...
To learn more on the different Android and iOS versions supported, refer to the .
...
* **An integration key**: to enable the Adobe Campaign platform to identify the mobile application.
  ...
  >[!NOTE]
  >
  >This integration key is entered in the Adobe Campaign console, in the **Information** tab of service dedicated to the mobile application. Refer to [General information](../../delivery/using/setting-up-mobile-app-channel.md#-general-information).
...
1. Create a **Mobile application** type [information service](../../delivery/using/setting-up-mobile-app-channel.md#creating-the-service-and-collecting-subscriptions) for the Neotrips mobile application.
...
1. Define a **Label** and an **Internal name**.
1. Go to the **Type** field and select **Mobile application**.

   >[!NOTE]
   >
   >The default **Subscriber applications (nms:appSubscriptionRcp)** target mapping is linked to the recipients table. If you want to use a different target mapping, you need to create a new target mapping and enter it in the **Target mapping** field of the service. For more on creating target mapping, refer to the [Configuration guide](../../configuration/using/about-custom-recipient-table.md).

1. Then click the **Add** button to define the various versions of your mobile application (iOS, Android). 
...
1. Define the mapping in the **Subscription parameters** tab.

   >[!CAUTION]
   >
   >Make sure the configuration names in the **Subscription parameters** tab are the same as those in the mobile application code. Refer to the [Integrating Campaign SDK into the mobile application](../../delivery/using/setting-up-mobile-app-channel.md#integrating-campaign-sdk-into-the-mobile-application) section.
...
Click the **Enter the certificate...** link then select the authentication certificate and enter the password that was provided by the mobile application developer.
...
1. Go to **Profiles and Targets** > **Services and Subscriptions**.
1. Click **New** to create a service.
1. In the **Edit** tab, select **Mobile application** as the **Type** and **Subscriber application** (nms:appSubscriptionRcp) as the **Target mapping**.
1. In the **List of mobile applications that use the service**, add a new application and select **Create an Android application**.
1. Click **Next**.
1. In the **Information** tab of the creation wizard, enter a label.
1. In the **Application variables** field, add the parameters that you want to use for sending a rich push:
...
1. Click **Finish** and save the service.
...
1. Duplicate the **Deliver on Android** template.
1. Change the label and click **Continue**.
1. Click the **To** link to target the application's subscribers.
1. Change the **Target mapping** to **Subscriber applications (nms:appSubscriptionRcp)**.

   ![](assets/nmac_rich_android_target_mapping.png)

1. Click **Add**, select **Subscribers of an Android mobile application** and click **Next**.
...
1. Click **New**.
1. Select the delivery template that you just created and click **Continue**.
1. In the **Application variables** field, add the values of your choice for the different parameters.

   ![](assets/nmac_rich_android_delivery.png)

1. Click **Save** and send your delivery.
