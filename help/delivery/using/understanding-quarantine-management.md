...
uuid: 3c5e75ad-38a7-41a8-a306-1ae6ad78ef9e
contentOwner: sauviat
products: SG_CAMPAIGN/CLASSIC
content-type: reference
discoiquuid: 6db4daaa-203d-474d-8ffd-a290606739b3
index: y
internal: n
snippet: y
---
...
Quarantined addresses for a specific delivery are listed during the delivery preparation phase, in the delivery logs of the delivery dashboard (see [Delivery logs and history](../../delivery/using/understanding-quarantine-management.md#delivery-logs-and-history)).

### Identifying quarantined addresses for the entire platform {#identifying-quarantined-addresses-for-the-entire-platform}

Administrators can list the addresses in quarantine for the entire platform from the **Administration > Campaign Management > Non deliverables Management > Non deliverables and addresses** node. This menu lists quarantined elements for **email**, **SMS** and **Push notification** channels.  
...
Adobe Campaign manages quarantine according to the delivery failure type and the reason assigned during error messages qualification (see [Bounce mail qualification](../../delivery/using/understanding-quarantine-management.md#bounce-mail-qualification)) and [Delivery failure types and reasons](../../delivery/using/understanding-quarantine-management.md#delivery-failure-types-and-reasons).
...
* **Soft error**: soft errors do not send immediately an address to quarantine, but they increment an error counter. When the error counter reaches the limit threshold, the address goes into quarantine. In the default configuration, the threshold is set at five errors, where two errors are significant if they occur at least 24 hours apart. The address is placed in quarantine at the sixth error. The error counter threshold can be modified. For more on this, refer to [Retries after a delivery temporary failure](../../delivery/using/understanding-quarantine-management.md#retries-after-a-delivery-temporary-failure).
...
>[!NOTE]
>
>The failure types and reasons for failure are the same as for emails. See [Delivery failure types and reasons](../../delivery/using/understanding-quarantine-management.md#delivery-failure-types-and-reasons).  
...
* The second part (**Generic** in this example) of the error message refers to the name of the SMSC implementation such as defined in the **SMSC implementation name** field of the SMS external account. See [this page](../../delivery/using/sms-channel.md#creating-an-smpp-external-account).
...
* The third part (**DELIVRD** in this example) of the error message corresponds to the status code retrieved from the SR using the status extraction regex defined in the SMS external account.
  ...
  This regex is specified in the **SMSC specificities** tab of the external account. See [this page](../../delivery/using/sms-channel.md#creating-an-smpp-external-account).
