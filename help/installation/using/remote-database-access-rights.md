...
|:-:|:-:|:-:|:-:|:-:|:-:|
| **Connecting to remote database**  | CONNECT privilege| Creating a user tied to a remote host who has ALL PRIVILEGES| No permission required to use the CONNECT statement | No privilege required | CONNECT privilege |
| **Creating tables**  | CREATE TABLE or TABLE keyword | CREATE privilege| RESOURCE authority and CREATE permission| TABLE privilege| CREATE privilege |
|**Creating indexes**  | CREATE INDEX or INDEX keyword | INDEX privilege | RESOURCE authority and CREATE permission| INDEX privilege | CREATE privilege|
|  **Creating functions**  | CREATE FUNCTION or FUNCTION keyword | CREATE ROUTINE privilege| RESOURCE authority or DBA authority for Java functions | FUNCTION privilege | CREATE FUNCTION privilege |
|  **Creating procedures**  | CREATE PROCEDURE or PROCEDURE keyword | CREATE ROUTINE privilege| RESOURCE authority| PROCEDURE privilege | CREATE FUNCTION privilege |
| **Removing objects (tables, indexes, functions, procedures)**  | DROP <&nbsp;object&nbsp;>  or object related keyword| DROP privilege | Owning the object or DBA authority | DROP privilege | Owning the object  |
