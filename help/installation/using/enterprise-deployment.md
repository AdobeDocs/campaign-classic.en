...
uuid: 1dea192e-fd97-4fdd-a2a5-30169ba12104
contentOwner: sauviat
products: SG_CAMPAIGN/CLASSIC
content-type: reference
discoiquuid: c822f37a-6e45-4ba8-834f-211cf801a137
index: y
internal: n
snippet: y
---
...
* Firewall configured to open STMP (25), DNS (53), HTTP (80), HTTPS (443), SQL (1521 for Oracle, 5432 for PostgreSQL, etc.) ports. For further information, refer to section [Database access](../../installation/using/enterprise-deployment.md#database-access)
...
1. Once the Adobe Campaign server is installed, start the application server (web) using the command **nlserver web -tomcat** (the Web module enables you to start Tomcat in standalone Web server mode listening on port 8080) and to make sure Tomcat starts correctly:
    ...
    * For Linux: [First start-up of the server](../../installation/using/enterprise-deployment.md#first-start-up-of-the-server),
    * For Windows: [First start-up of the server](../../installation/using/enterprise-deployment.md#first-start-up-of-the-server).
...
1. Change the **internal** password using the command:
   ...
   For more on this, refer to [Internal identifier](../../installation/using/enterprise-deployment.md#internal-identifier).
...
1. Create the **demo** instance with the DNS masks for tracking (in this case, **tracking.campaign.net**) and access to client consoles (in this case, **console.campaign.net**). There are two ways of doing this:
    ...
    * Create the instance using command lines:
      ...
      For more on this, refer to [Creating an instance](../../installation/using/enterprise-deployment.md#creating-an-instance).
...
1. Edit the **config-demo.xml** file (created via the previous command and located next to the **config-default.xml** file), check that the **mta** (delivery), **wfserver** (workflow), **inMail** (rebound mails) and **stat** (statistics) processes are enabled, then configure the address of the **app** statistics server:
   ...
   For more on this, refer to [Enabling processes](../../installation/using/enterprise-deployment.md#enabling-processes).
...
1. Start the Adobe Campaign server (**net start nlserver6** in Windows, **/etc/init.d/nlserver6 start** in Linux) and run the command **nlserver pdump** once more to check for presence of all enabled modules.
   ...
   ```
   12:09:54 >   Application server for Adobe Campaign Version 6.X (build XXXX) of DD/MM/YYYY
...
1. Start the website and test the redirection from the URL: [https://tracking.campaign.net/r/test](https://tracking.campaign.net/r/test)
    ...
    * For Linux: [Launching the Web server and testing the configuration](../../installation/using/enterprise-deployment.md#launching-the-web-server-and-testing-the-configuration),
    * For Windows: [Launching the Web server and testing the configuration](../../installation/using/enterprise-deployment.md#launching-the-web-server-and-testing-the-configuration).
