...
uuid: f8f689ab-b1a4-4f37-8a70-65087fed4fb2
contentOwner: sauviat
products: SG_CAMPAIGN/CLASSIC
content-type: reference
discoiquuid: 0a381ba5-7a78-4b41-8145-7eb8fd230a78
index: y
internal: n
snippet: y
---
...
Answers to forms can be saved in a field of the database or temporarily in a local variable. The storage mode for answers is chosen during field creation. It can be edited via the **Edit storage...** link.
...
To save the data in an existing field of the database, click the **Edit expression** icon and select it from the list of available fields. 

![](assets/s_ncs_admin_survey_storage_type1.png)

>[!NOTE]
>
>The default reference document is the **nms:recipient** schema. To view it or to choose a new one, select the form from the list, and click the **Properties** button.
...
It is stored in a variable which must be selected when the drop-down box is created, or via the **Edit storage...** link. 

![](assets/s_ncs_admin_survey_no_storage_variable2.png)

You can display existing variables and create new ones via the **Edit variables...** link. Click the **Add** button to create a new variable.
...
>[!NOTE]
>
>For each form, you can create variables upstream. To do this, select the form and click the **Properties** button. The **Variables** tab contains the local variables for the form.

**Example of local storage with conditioning**

In the above example, the container that includes data concerning private vehicles is displayed only if the **Private** option is selected from the drop-down list, as indicated in the visibility condition:
...
* If the Web form is accessed via a link sent in an email, and if the user accessing the application is already in the database, you can check the **Update the preloaded record** option. For more on this, refer to [Delivering a form via email](../../web/using/web-forms-answers.md#delivering-a-form-via-email).

  In this case, Adobe Campaign uses the encrypted primary key of the user profile, a unique identifier assigned to each profile by Adobe Campaign. You need to configure the information to preload via the preloading box. For more on this, refer to [Pre-loading the form data](../../web/using/web-forms-answers.md#pre-loading-the-form-data).
...
By default, the data is imported into the database by an **Update or insertion** operation: if it exists in the database, the element is updated (for example, the selected newsletter or the e-mail address entered). If it does not exist, the information is added.
...
>[!NOTE]
>
>Possible operations are: **Simple reconciliation**, **Update or insertion**, **Insertion**, **Update**, **Deletion**.  
